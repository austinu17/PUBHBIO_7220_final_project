...
logit balanceprob age stroke gender i.raceth dementia depress fall age_x_dementia age_x_raceth_2 if validation == 0 
...
egen h_rank = rank(-h)
sort h_rank 
l spid h h_rank if h_rank <= 5, sepby(h_rank)
egen dx2_rank = rank(-dx2)
sort dx2_rank 
l spid dx2 dx2_rank if dx2_rank <= 5, sepby(dx2_rank)
egen db_rank = rank(-db)
sort db_rank 
l spid db db_rank if db_rank <= 5, sepby(db_rank)

*HIGH LEVERAGE VALUE POINT REMOVED FOR >20% CHANGE IN BETAS*
drop if spid==10005469
drop if spid==10006898
...
quietly logit balanceprob age stroke gender i.raceth dementia depress fall age_x_dementia age_x_raceth_2 if validation == 0
